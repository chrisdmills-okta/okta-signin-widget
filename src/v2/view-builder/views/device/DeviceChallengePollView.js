...
const cancelPolling = (appState, cancelReason) => {
  const actionParams = {};
        ...
        return checkPort()
          ...
          .done(() => {
            return onPortFound()
              ...
              .fail(() => {
                cancelPolling(this.options.appState, AUTHENTICATION_CANCEL_REASONS.OV_ERROR);
              });
          })
          .fail(onFailure);
      };
        ...
        probeChain = probeChain
          ...
          .catch(() => {
            ...
            if (countFailedPorts === ports.length) {
              ...
              cancelPolling(this.options.appState, AUTHENTICATION_CANCEL_REASONS.LOOPBACK_FAILURE);
            }
          });
      });
    },
    ...
    if (this.isFallbackApproach() && !this.isFallbackDelayed()) {
      ...
      this.backLink = this.add(Link, {
        options: {
          ...
          clickHandler: () => {
            cancelPolling(this.options.appState, AUTHENTICATION_CANCEL_REASONS.USER_CANCELED);
          },
        }
      }).last();
    }
